...
=======
    highlight: zenburn
>>>>>>> 23cbf2393a0fcb3e067b64eaea77a4332f207b83
    smart: no
...
***
# Repositórios online e interfaces gráficas
...
- Salvar alterações
- Dar `commit` no repositório local via linha de comando, QtCreator e RStudio
- Dar `pull` no repositório local via  linha de comando, QtCreator e RStudio (para evitar problemas de conflito caso alguem mais tenha alterado o projeto)
- Resolver problemas de `merge`
- Dar `push` no `remote` via  linha de comando, QtCreator e RStudio
...
`touch` é um comando que cria um arquivo vazio. Você também pode criar um arquivo com seu editor favorito ou colar um existente.
...
***
No GitHub, clique em `New repository` para criar um repositório novo ou em `import repository` para copiar um que já existe em outro site online
...
`git pull` é o comando que puxa o conteúdo do remoto para seu loca, e tenta fazer o `merge` automaticamente. Se ele não conseguir, ele vai te avisar. Veja como lidar com isso na sessão de `branches`, abaixo.
...
Um branch é como uma linha de desenvolvimento alternativa, com seus próprios commits, independente de outros branches. Podemos pensar um branch como sendo um evento de cladogênese, em que cada ramo pode ter seus traços (arquivos) alterados independentemente,e novos traços (arquivos) podem surgir ou ser apagados sem afetar os outros ramos.
